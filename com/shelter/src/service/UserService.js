                        ...
                        userPackageDao.getPackageInfo(master, packageType, function (flag, msg, rs) {
                            if (flag) {
                                ...
                                if (Number(moneyneed) >= Number(moneyRequired) * packageNum) {
                                    //插入用户卡包
                                    userPackageDao.addUserPackages(master, userId, packageName, packageType, packageNum, function (flag, msg, rs) {
                                        if (flag) {
                                            //减少用户金币数量
                                            userDao.updateUserMoney(master, -Number(moneyRequired * packageNum), userId, function (flag, msg, rs) {
                                                if (flag) {
                                                    //没问题就提交
                                                    logger.debug("transaction commit");
                                                    master.commit(function (err) {
